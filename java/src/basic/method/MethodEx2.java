...
/*
 ...
 3.매개값을 몇개 받아서 사용할지는 메서드를 선언할 때 선언부에서
   ()안에 미리 개수와 타입을 지정한다. (, , , , ...max 255)
   
 4.매개변수의 이름을 지을 때는 호출자가 선언부로 돌아와서
   매개값을 확인하지 않게, 어떤 값을 담는 변수인지 
   그 뜻을 알기 쉽게 지어주면 편하게 이용할 수 있다.
   
 5.매개변수를 하나도 선언하지 않을 수도 있다. 이 때는 메서드의 선언부에
   () 내부를 비워둔다. 그리고 호출할 때도 ()를 비워서 호출한다.
 */
public class MethodEx2 {
	...
	//정수 n개를 전달하면 그 정수들의 총 합을 리턴하는메서드
	...
	// , 로 나열되어 들어오는 여러개의 값을 배열로 묶어서 내부로 전달한다.
	...
	public static void main(String[] args) {
		...
		total = calcRangeTotal(100, 10);
		System.out.println(total);
		
		System.out.println(selectRandFood());
		...
		//가변 파라미터에 값을 보낼 때는 ,로 쭉 나열하거나
		//배열로 포장해서 보내도 된다.
		sum = calcNumberTotal2(1,2,3,4,5);
		System.out.println("누적합: "+sum);
