	...
	public static void main(String[] args) {
		/*
		 ...
		 - String 객체는 처음 초기화된 데이터에 변화를 주어야 하는 상황에서 
		   기존 객체를 활용하지 않고 새로운 객체를 계속해서 생선한다.

		 # StringBuilder, StringBuffer (자바 5버전부터 사용 가능)
		 - String 클래스 단점으로 인한 메모리 과부화 및 속도가 느려지는 현상을
		   개선하기 위해서 StringBuilder가 새롭게 추가되었다.
		   
		 - StringBuilder는 기존 객체를 계속 재활용하면서 실제 객체 내부의 값을
		   변경하게 된다.
		 
		 - StringBuilder, StringBuffer는 메서드가 동일하다.
		   StringBuilder -> 단일 스레드
		   StringBuffer -> 다중스레드에서 문자열을 공유해야 할 때
		 */
		String str = "hello";
...
//		System.out.println("sb의 주소값"+sb.hashCode());
		...
		//특정 인덱스 범위의 문자열을 수정하는 메서드: replace(begin, end, 문자열)
		sb.replace(6, 8, "your");
		System.out.println(sb);
