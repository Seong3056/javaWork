...
/*
 ...
 - 상속을 적용시키려면 자식 클래스 선언부의 클래스 이름 뒤에 
   키워드 extends를 쓰고, 물려받을 부모 클래스의 이름을 작성한다.
 */
public class Warrior extends Player {
	...
	/*
	 ...
	 - 부모가 물려주는 메서드는 모든 자식들에게 다 맞게 설계되기 힘들기 때문에
	   부족한 점이 있거나, 맞지 않는 경우는 자식 클래스 쪽에서
	   내용을 추가하거나 수정할 수 있습니다. 이를 오버라이딩 이라고 한다.
	   
	 - 자식 클래스에서 부모가 물려준 메서드를 새롭게 재정의한다면
	   자식이 수정한 메서드가 우선적으로 호출된다.
	 
	 - 오버라이딩의 규칙(3가지 모두 만족해야 오버라이딩 인정)
	 1.부모가 물려주는 메서드와 이름이 일치
	 2.부모가 물려주는 메서드와 리턴 타입 일치
